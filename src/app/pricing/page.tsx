...
import { ArrowLeft, BadgeInfo, ChevronDown, ChevronUp, DollarSign, ExternalLink, Search } from "lucide-react";
      ...
      <header className="h-14 border-b border-border bg-card/80 backdrop-blur-sm flex items-center justify-between px-6 shrink-0">
        ...
        <div className="flex items-center gap-2">
          <Button variant={compareMode ? "default" : "outline"} size="sm" onClick={() => setCompareMode(!compareMode)}>
            <BadgeInfo className="w-3.5 h-3.5 mr-2" /> Compare
          </Button>
          <Link href="/docs/models" className="flex items-center gap-2 px-3 py-1.5 text-xs border border-input rounded-md hover:bg-accent hover:text-accent-foreground transition-colors">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
        ...
        {!search && selectedProvider === "all" && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <MiniStatCard label="Cheapest Prompt" value={`${formatPrice(getMinPrice(pricingData))}/1M`} />
            <MiniStatCard label="Most Expensive" value={`${formatPrice(getMaxPrice(pricingData))}/1M`} />
            <MiniStatCard label="Providers" value={String(providers.length)} />
            <MiniStatCard label="Total Models" value={String(pricingData.length)} />
          </div>
        )}
      </div>
    </div>
